...
describe('Production API', () => {
  ...
  describe('GET /api/production/board', () => {
    test('should get production board (Kanban view)', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/board', authToken);
  ...
  describe('GET /api/production/stats', () => {
    test('should get production statistics', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/stats', authToken);
  ...
  describe('GET /api/production/tasks', () => {
    test('should get all production tasks', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/tasks', authToken);
    ...
    test('should filter tasks by status', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/tasks?status=pending', authToken);
    ...
    test('should support pagination', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/tasks?page=1&limit=10', authToken);
  ...
  describe('POST /api/production/tasks', () => {
    test('should create production task with valid data', async () => {
      const response = await makeAuthenticatedRequest(app, 'POST', '/api/production/tasks', authToken, {
  ...
  describe('GET /api/production/tasks/:id', () => {
    test('should get task by valid ID', async () => {
      if (testTaskId) {
        const response = await makeAuthenticatedRequest(app, 'GET', `/api/production/tasks/${testTaskId}`, authToken);
    ...
    test('should return 404 for non-existent task', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/tasks/non-existent-id-123', authToken);
  ...
  describe('PUT /api/production/tasks/:id', () => {
    test('should update task with valid data', async () => {
      if (testTaskId) {
        const response = await makeAuthenticatedRequest(app, 'PUT', `/api/production/tasks/${testTaskId}`, authToken, {
    ...
    test('should return 404 for non-existent task', async () => {
      const response = await makeAuthenticatedRequest(app, 'PUT', '/api/production/tasks/non-existent-id-123', authToken, {
        title: 'Updated Title'
      });
  ...
  describe('PATCH /api/production/tasks/:id/status', () => {
    test('should update task status', async () => {
      if (testTaskId) {
        const response = await makeAuthenticatedRequest(app, 'PATCH', `/api/production/tasks/${testTaskId}/status`, authToken, {
          status: 'in_progress'
        });
    ...
    test('should return 404 for non-existent task', async () => {
      const response = await makeAuthenticatedRequest(app, 'PATCH', '/api/production/tasks/non-existent-id-123/status', authToken, {
        status: 'completed'
      });
  ...
  describe('PATCH /api/production/orders/:id/stage', () => {
    test('should update order stage', async () => {
      const response = await makeAuthenticatedRequest(app, 'PATCH', `/api/production/orders/${testOrderId}/stage`, authToken, {
        stage: 'designing',
    ...
    test('should return 404 for non-existent order', async () => {
      const response = await makeAuthenticatedRequest(app, 'PATCH', '/api/production/orders/non-existent-id-123/stage', authToken, {
        stage: 'production'
      });
  ...
  describe('GET /api/production/quality-checks', () => {
    test('should get all quality checks', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/quality-checks', authToken);
    ...
    test('should filter by order_id', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', `/api/production/quality-checks?order_id=${testOrderId}`, authToken);
  ...
  describe('POST /api/production/quality-checks', () => {
    test('should create quality check', async () => {
      const response = await makeAuthenticatedRequest(app, 'POST', '/api/production/quality-checks', authToken, {
        order_id: testOrderId,
  ...
  describe('POST /api/production/workflows', () => {
    test('should create production workflow', async () => {
      const response = await makeAuthenticatedRequest(app, 'POST', '/api/production/workflows', authToken, {
        order_id: testOrderId,
  ...
  describe('GET /api/production/workflows/order/:orderId', () => {
    test('should get workflows by order', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', `/api/production/workflows/order/${testOrderId}`, authToken);
  ...
  describe('POST /api/production/issues', () => {
    test('should create production issue', async () => {
      const response = await makeAuthenticatedRequest(app, 'POST', '/api/production/issues', authToken, {
        order_id: testOrderId,
        type: 'material',
        severity: 'high',
  ...
  describe('GET /api/production/schedule', () => {
    test('should get production schedule', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/schedule', authToken);
    ...
    test('should filter by date', async () => {
      ...
      const response = await makeAuthenticatedRequest(app, 'GET', `/api/production/schedule?date=${today}`, authToken);
  ...
  describe('GET /api/production/analytics', () => {
    test('should get production analytics (week)', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/analytics?period=week', authToken);
    ...
    test('should get production analytics (month)', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/analytics?period=month', authToken);
  ...
  describe('GET /api/production/templates', () => {
    test('should get production templates', async () => {
      const response = await makeAuthenticatedRequest(app, 'GET', '/api/production/templates', authToken);
  ...
  describe('PUT /api/production/tasks/:id/assign', () => {
    test('should assign task', async () => {
      if (testTaskId) {
        const response = await makeAuthenticatedRequest(app, 'PUT', `/api/production/tasks/${testTaskId}/assign`, authToken, {
    ...
    test('should return 404 for non-existent task', async () => {
      const response = await makeAuthenticatedRequest(app, 'PUT', '/api/production/tasks/non-existent-id-123/assign', authToken, {
        assigned_to: 'user-id'
      });
  ...
  describe('DELETE /api/production/tasks/:id', () => {
    ...
    test('should delete task', async () => {
      if (taskToDelete) {
        const response = await makeAuthenticatedRequest(app, 'DELETE', `/api/production/tasks/${taskToDelete}`, authToken);
    ...
    test('should return 404 for non-existent task', async () => {
      const response = await makeAuthenticatedRequest(app, 'DELETE', '/api/production/tasks/non-existent-id-123', authToken);
