  ...
  describe('GET /api/purchase-orders', () => {
    ...
    test('should support pagination', async () => {
      const response = await makeAuthenticatedRequest(
        app,
        'GET',
        '/api/purchase-orders?page=1&limit=5',
        authToken
      );
  ...
  describe('GET /api/purchase-orders/:id', () => {
    test('should get purchase order by valid ID', async () => {
      const response = await makeAuthenticatedRequest(
        app,
        'GET',
        `/api/purchase-orders/${testPurchaseOrderId}`,
        authToken
      );
    ...
    test('should return 404 for non-existent purchase order', async () => {
      const response = await makeAuthenticatedRequest(
        app,
        'GET',
        '/api/purchase-orders/non-existent-id-123',
        authToken
      );
  ...
  describe('PATCH /api/purchase-orders/:id/status', () => {
    test('should update purchase order status to ordered', async () => {
      const response = await makeAuthenticatedRequest(
        app,
        'PATCH',
        `/api/purchase-orders/${testPurchaseOrderId}/status`,
        authToken,
        {
          status: 'ordered',
        }
    ...
    test('should return 404 for non-existent purchase order', async () => {
      const response = await makeAuthenticatedRequest(
        app,
        'PATCH',
        '/api/purchase-orders/non-existent-id-123/status',
        authToken,
        {
          status: 'ordered',
        }
