  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        db.db
          .prepare(
            "INSERT OR IGNORE INTO revoked_tokens (token_jti, user_id, expires_at) VALUES (?, ?, datetime(?, 'unixepoch'))"
          )
          .run(request.user.jti, request.user.id, request.user.exp);
      } catch (error) {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        db.db
          .prepare(
            "UPDATE users SET reset_token = ?, reset_token_expires = datetime('now', '+1 hour'), updated_at = CURRENT_TIMESTAMP WHERE id = ?"
          )
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        const user = db.db
          .prepare(
            "SELECT * FROM users WHERE id = ? AND reset_token = ? AND reset_token_expires > datetime('now')"
          )
