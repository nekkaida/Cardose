    ...
    async (request, reply) => {
      try {
        ...
        if (!deviceId || !lastSyncTimestamp) {
          return reply
            .status(400)
            .send({ error: 'Device ID and last sync timestamp are required' });
        }
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        const history = await syncService.getSyncHistory(deviceId, limit ? parseInt(limit) : 50);
    ...
    async (request, reply) => {
      try {
        ...
        if (!chosenVersion || !['existing', 'incoming'].includes(chosenVersion)) {
          return reply
            .status(400)
            .send({ error: 'Invalid chosen version. Must be "existing" or "incoming"' });
        }
