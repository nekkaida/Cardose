...
import { CATEGORIES, CATEGORY_I18N, INPUT_LIMITS } from './inventoryHelpers';
...
const buildInitialFormData = (editingItem: InventoryFormModalProps['editingItem']) => ({
...
const InventoryFormModal: React.FC<InventoryFormModalProps> = ({
  ...
  const [formData, setFormData] = useState(() => buildInitialFormData(editingItem));
  const [formError, setFormError] = useState<string | null>(null);

  // Reset form state every time the modal opens (or the editingItem changes)
  // so that reopening with the same key starts from a clean slate.
  useEffect(() => {
    if (open) {
      setFormData(buildInitialFormData(editingItem));
      setFormError(null);
    }
  }, [open, editingItem]);
  ...
  const notesThreshold = Math.floor(INPUT_LIMITS.NOTES_MAX * 0.8);
      ...
      >
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="form-unit" className="mb-1 block text-sm font-medium text-gray-700">
                {t('inventory.unit')}
              </label>
              <input
                id="form-unit"
                ...
                maxLength={INPUT_LIMITS.UNIT_MAX}
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label
                htmlFor="form-reorder-level"
                className="mb-1 block text-sm font-medium text-gray-700"
              >
                {t('inventory.reorderLevel')}
              </label>
              <input
                id="form-reorder-level"
                type="number"
            ...
            <div>
              <label
                htmlFor="form-current-stock"
                className="mb-1 block text-sm font-medium text-gray-700"
              >
                {t('inventory.currentStock')}
              </label>
              <input
                id="form-current-stock"
                type="number"
          ...
          <div>
            <label htmlFor="form-supplier" className="mb-1 block text-sm font-medium text-gray-700">
            ...
            <input
              ...
              maxLength={INPUT_LIMITS.SUPPLIER_MAX}
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
              placeholder={t('inventory.supplierName')}
            />
          </div>
          <div>
            <label htmlFor="form-notes" className="mb-1 block text-sm font-medium text-gray-700">
              {t('inventory.notes')}
            </label>
            <textarea
              id="form-notes"
            ...
            />
            {formData.notes.length > notesThreshold && (
              <p className="mt-1 text-xs text-gray-500">
                {formData.notes.length}/{INPUT_LIMITS.NOTES_MAX}
              </p>
            )}
          </div>
        </div>
